# -*- coding: utf-8 -*-
"""Project_part_2_FNN.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Gpzp-kyixbzIwFvtZmHELF3k5XTEFrzi

Explanation for Each Section Data Loading and Preprocessing:

The target variable (y) and features (X) are separated after the dataset is imported from an Excel file.
Train_test_split is employed to partition the data into training and test sets.
StandardScaler is employed to normalise numerical features.

RBM Class:

The Restricted Boltzmann Machine is implemented by the RBM class for feature extraction.
The class encompasses techniques for performing Gibbs sampling, training the RBM, sampling visible units given hidden units, and sampling hidden units given visible units.

FNN Class:
The FNN class is designed to implement a Feedforward Neural Network for classification.
Methods for initialising the network and conducting the forward pass are included in the class.

Training Functions:
The DataLoader provided is utilised by the train_fnn function to train the FNN model.
PyTorch DataLoader data is prepared by the prepare_dataloader function.

Grid Search and Cross-Validation:
K-Fold Cross-Validation is implemented to execute a grid search across various n_hidden values.
For each n_hidden value, the average validation accuracies are computed and visualised.

Final Training and Evaluation:
The optimal n_hidden value is employed to train the RBM and FNN on the entire training set.
The scaler and trained models are retained.
The FNN is assessed on the test set, and a variety of performance metrics are computed and displayed.
The confusion matrix and ROC curve are illustrated.

In comparison to the original features:
The original scaled features are also used to train and evaluate the FNN.
In order to ascertain whether RBM enhances performance, the performance of models employing RBM features is contrasted with that of models employing original features.
"""
